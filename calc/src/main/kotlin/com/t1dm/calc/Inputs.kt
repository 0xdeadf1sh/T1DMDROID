...
data class IobSnapshot(
    ...
    val cobG: Double,
    /**
     * The log-gap mark. `MAX(MIN(tsMs, loggedAtMs))` over the dose store, not `MAX(tsMs)` — it
     * answers "how long since the phone was TOLD about insulin", not "what timestamp is on the
