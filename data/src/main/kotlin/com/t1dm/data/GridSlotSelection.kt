 ...
 *  3. **Then the sample received nearest the slot instant** — `|rxWallMs − tsMs|`. This is the whole
 *     point: the slot claims to be the sensor's value at that instant, so the sample nearest it is
 *     the one that best supports the claim.
 *  4. **Then the earlier reception.** Equidistant is reachable — a slot's own five minutes are
 *     symmetric about it, so a sample 60 s early and one 60 s late tie — and something has to break
 *     it. The earlier one wins, which means the incumbent normally keeps the slot and a settled
 ...
 *
 * Rules 2–4 are a total order on `(suppressed, distance, rxWallMs)` over samples with distinct
 * receive instants, so the surviving row is a function of the SET of samples for the slot and not of
 * the order they were persisted in — which is what makes an out-of-order or retried write land on
 * the same answer as a clean run.
 *
